...
ノートブックの状態はリセットされたので、レッスン用の設定を初期化する必要があります。

注：このレッスンの後半ではデータベースは再作成**しません**。
...
しかし、今まで使用してきたデータベースがデフォルトで使用されるようにこのセッションを設定する必要があります。
...
次の2つのセルを実行して次のことを確認します。
1. テーブル **`external_table`** はまだ存在しています。
2. ビュー **`view_delays_abq_lax`** はまだ存在しています。
3. テンポラリビュー **`temp_view_delays_gt_120`** は存在**しません**。
3. グローバルテンポラリビュー **`global_temp_view_dist_gt_1000`** は、特別な **`global_temp`** データベースに存在します。

<img src="https://files.training.databricks.com/images/icon_hint_24.png" /> ヒント：再び、前のノートブックに戻って **`SHOW TABLES`** <br/>を実行してみると、現在のデータベースの3つすべてのテーブルとビューが表示されます。
...
前述したように、テンポラリビューは、Sparkセッションに紐づかれているため、アクセスができません…
* クラスタを再起動した後
* クラスタのデタッチと再アタッチの跡
* PythonパッケージをインストールしてPythonのインタプリタが再起動された後
* または別のノートブックから

…グローバルテンポラリビューという特例を除いて。

-- COMMAND ----------

-- MAGIC %md <i18n value="1cf155bd-1953-49e3-8c2b-f2db695f93af"/>
グローバルテンポラリは、通常のテンポラリビューと同じく動作しますが、1つ重要な違いがあります。

グローバルテンポラリは、 **`クラスタ`** に存在する **`global_temp`** データベースに追加されます。

クラスタが動作している限り、このデータベースは永続化され、クラスタにアタッチされているいかなるノートブックもそのグローバルテンポラリビューにアクセスできます。

次のセルを実行してこの動作を実際確認できます。
...
クラスタを再起動すると、グローバルテンポラリビューは「失われます」。

今はしなくても良いですが、クラスタを再起動すると、テーブルがもう存在しないため、上のSELECT文は失敗します。

-- COMMAND ----------

-- MAGIC %md <i18n value="30eadd27-c9ea-4c5c-8c9b-7cfa9476a054"/>
## 共通テーブル式（CTE）（Common Table Expressions (CTEs)）
CTEはさまざまな場所で使用されます。 以下は、CTEを照会で使用できるさまざまな方法のくつかの例です。 はじめに、CTEを使用して複数の列エイリアスを作成する例を紹介します。
