...
# Databricks上のデータベースとテーブル（Databases and Tables on Databricks）
このデモンストレーションでは、データベースとテーブルを作成して調べます。

## 学習目標（Learning Objectives）
このレッスンでは、以下のことが学べます。
* Spark SQL DDLを使用してスキーマとテーブルを定義する
*  **`LOCATION`** キーワードがデフォルトのストレージディレクトリにどのような影響を与えるかを説明する



**リソース**
* <a href="https://docs.databricks.com/user-guide/tables.html" target="_blank">スキーマとテーブル - Databricksドキュメント</a>
...
## Hive変数を使用する（Using Hive Variables）

このパターンは基本、Spark SQLではお勧めしませんが、このノートブックでは現在のユーザーのアカウントのメールアドレスから得られた文字列を置き換えるためにHive変数を使用します。
...
共有ワークスペースで作業している可能性があるため、スキーマが他のユーザーと競合しないようにするためにこのコースでは、あなたのユーザー名から得られた変数を使用します。 繰り返しますが、このHive変数の使用は、開発のための良い習慣というよりはレッスン環境のための裏技だと考えてください。
...
最初のスキーマの場所は、 **`dbfs:/user/hive/warehouse/`** にあるデフォルトの場所で、スキーマのディレクトリは **`.db`** の拡張子が付いているスキーマの名前であることにご注意ください。
...
2個目のスキーマの場所は、 **`LOCATION`** キーワードの後ろに指定されているディレクトリであることにご注意ください。
...
デフォルトの場所を使用してスキーマにテーブルを作成して、データを挿入します。

スキーマを推測するためのデータがないため、スキーマを指定する必要があることにご注意ください。
...
テーブルの詳細な説明で（結果で下にスクロール）場所を見つけられます。
...
デフォルトでは、場所を指定しないスキーマのマネージドテーブルは **`dbfs:/user/hive/warehouse/<database_name>.db/`** ディレクトリに作成されます。

Deltaテーブルのデータとメタデータは予想通り、その場所に保存されていることが分かります。
...
テーブルのディレクトリとそのログ、およびデータのファイルが削除されていることにご注意ください。 スキーマのディレクトリのみが残りました。
...
カスタムの場所を使用してスキーマにテーブルを作成し、データを挿入します。
...
予想どおり、このマネージドテーブルは、スキーマ作成時に **`LOCATION`** キーワードで指定されたパスに作成されました。 したがって、テーブルのデータとメタデータはこちらのディレクトリに保持されます。
...
テーブルのフォルダとログファイル、データファイルが削除されていることにご注意ください。
...
## テーブル（Tables）
サンプルデータを使用して外部（アンマネージド）テーブルを作成します。

CSV形式のデータを使用します。 好きなディレクトリに指定した **`LOCATION`** のDeltaテーブルを作成します。
...
このレッスンの作業ディレクトリのテーブルデータの場所にご注意ください。
...
テーブルの定義はもうメタストアには存在しませんが、その元になっているデータは残っています。
