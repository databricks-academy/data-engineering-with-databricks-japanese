...
-- MAGIC 
-- MAGIC  **`sales_orders_raw`** は、 **retail-org/sales_orders** データセットからJSONデータを段階的に取り込みます。
-- MAGIC 
...
-- MAGIC ### customers
-- MAGIC 
-- MAGIC  **`customers`** は、 */databricks-datasets/retail-org/customers/* にあるCSV顧客データを表します。 
-- MAGIC  
-- MAGIC  このテーブルは購入記録に基づいた顧客データを調べるために、この後すぐに結合操作で使用します。
