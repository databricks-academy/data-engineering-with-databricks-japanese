...
Databricks SQLでは、多くの追加カスタムメソッド（Delta Lake SQLの構文全体も含めて）を備えたANSIに準拠したSQLを使用できますが、一部のシステムから移行してきたユーザーは、機能（特に制御流れやエラーハンドリングの機能）が足りないと感じるかもしれません。

Databricksノートブックを使用すると、ユーザーはSQLとPythonを書いてロジックをセルごとに実行できます。 PySparkは、SQLクエリの実行を幅広くサポートしており、テーブルとテンポラリビューと簡単にデータを交換できます。

一部のPythonの概念をマスターするだけで、SQLに精通しているエンジニアやアナリストは新しい強力なデザイン方法を身に着けられます。 このレッスンでは、言語全体を教えるのではなく、Databricksでより拡張可能なSQLプログラムを即座に書くのに活用できる機能に焦点を当てています。

## 学習目標（Learning Objectives）
このレッスンでは、以下のことが学べます。
* 複数行のPython文字列の表示と操作
* 変数と関数の定義
* 変数置き換えのためののf文字列の使用方法

# COMMAND ----------

# MAGIC %md <i18n value="ae07e616-51d4-4ccb-ae28-6c22b7a203e6"/>
## 文字列（Strings）
一重引用符（ **`'`** ）または二重引用符（ **`"`** ）で囲われている文字は文字列です。
...
文字列がどのように表示されるかを確認するには **`print()`** を呼び出します。
...
文字列を三重引用符（ **`"""`** ）で囲むことで、複数行での使用が可能になります。
...
これのおかがで、SQLクエリをPython文字列に変えるのは簡単です。
...
SQLをPythonセルから実行する際、 **`spark.sql()`** に引数として文字列を渡します。
...
クエリを通常のSQLノートブックと同じ方法で表示するには、この関数に **`display()`** を呼び出します。
...
**注**：Pythonの文字列しか載っていないセルを実行しても、文字列が表示されるだけです。 文字列で **`print()`**  を使用してもノートブックにレンダリングされるだけです。

Pythonを使用してSQLが含まれている文字列を実行するには、 **`spark.sql()`** を呼び出す際、文字列を渡す必要があります。

# COMMAND ----------

# MAGIC %md <i18n value="6182d5b6-59e8-401f-acb1-d4d57c5b6018"/>
## 変数（Variables）
Pythonの変数は **`=`** を使用して割り当てできます。

Python変数は英字で始まる必要があり、英数字とアンダースコアのみを含められます。 （アンダースコアで始まる変数名は有効ですが、通常は特別な場面で使用されます。）

多くのPythonプログラマーは、すべての変数に小文字とアンダースコアのみを使用するスネークケースを好みます。

以下のセルでは **`my_string`** という変数を作成します。
...
ここの出力は、セルに **`"This is a string"`** と書いて実行した場合のと同じ出力になります。

表示時に表示されるとおり、引用符は文字列の一部ではないことに注意してください。
...
この変数は、文字列と同じように使用できます。

文字列の連結（2つの文字列を結合すること）は、 **`+`** を使用して実行できます。
...
## 関数（Functions）
関数を使用すると、ローカル変数を引数として指定して、カスタムロジックを適用できます。 キーワード **`def `** に続いて関数名、および括弧で囲んで関数に渡したい変数引数を記載して関数を定義します。 そして、関数ヘッダの最後に **`:`** があります。

注：Pythonでは、インデントが重要です。 以下のセルで、関数のロジックが左マージンからインデントされていることがわかります。 このレベルにインデントされたコードはすべて関数の一部です。

以下の関数は、1つの引数（ **`arg`** ）を取り、それを表示します。
...
多くの場合、他の場所で使用するために関数の結果を返したいことがあります。 そのために、 **`return`** キーワードを使用します。

以下の関数は、引数を連結して新しい文字列を作成します。 関数と引数の両方に、変数と同じように任意の名前を付けられることに注意してください（同じルールに従います）。
...
それを変数に割り当てると、他の場所で再利用するために出力をキャプチャできます。
...
この変数には関数は含まれていません。関数の結果（文字列）のみが含まれています。
...
## f-文字列（F-strings）
Python文字列の前に文字 **`f`** を追加することで、中括弧（ **`{}`** ）内に変数や評価するPythonコードを挿入することで、それらをPython文字列に挿入できます。

以下のセルを評価して、文字列変数の置換を確認しましょう。
...
これを三重引用符と組み合わせると、以下のようにパラグラフまたはリストをフォーマットできます。
