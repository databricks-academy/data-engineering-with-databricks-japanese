...
# MAGIC 
# MAGIC 追加された3層の名前空間機能は、デフォルトの **`hive_metastore`** とダッシュボードやクエリなどに使用される **`samples`** カタログを切り替えることである程度下見できます。 
# MAGIC 
...
# MAGIC 
# MAGIC **権限**タブをクリックし、現在の権限所有者を確認します（ワークスペースの設定によっては、一部の権限がカタログ上の設定から継承されている場合があります）。
