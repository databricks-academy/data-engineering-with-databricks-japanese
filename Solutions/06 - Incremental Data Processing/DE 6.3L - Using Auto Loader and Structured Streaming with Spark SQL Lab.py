...
# Spark SQLを用いたAuto Loaderと構造化ストリーミングを使用する（Using Auto Loader and Structured Streaming with Spark SQL）

## 学習目標（Learning Objectives）
このラボでは、以下のことが学べます。
* Auto Loaderを利用してデータを取り込む
* ストリーミングデータを集約する
* Deltaテーブルにデータをストリームする
...
## ストリーミング読み取りを構成する（Configure Streaming Read）

このラボでは、 **retail-org/customers** の顧客関連csvデータのコレクションを使います。

スキーマ推論を使って<a href="https://docs.databricks.com/spark/latest/structured-streaming/auto-loader.html" target="_blank">Auto Loader</a>でこのデータを読み取ります（ **`customers_checkpoint_path`** を使ってスキーマ情報を格納する）。  **`customers_raw_temp`** というストリーミングテンポラリビューを作成します。
...
## ストリーミング集約を定義する（Define a streaming aggregation）

CTAS構文を使って、 **`customer_count`** のフィールドにおける **`state`** ごとの顧客数をカウントする **`customer_count_by_state_temp`** という新しいストリーミングビューを定義します。
...
## 集約されたデータをDeltaテーブルに書き込む（Write aggregated data to a Delta table）

データを **`customer_count_by_state_temp`** ビューから **`customer_count_by_state`** というDeltaテーブルにストリームします。
 ...
 **`customer_count_by_state`** テーブルを照会します（これはストリーミングクエリではありません）。 結果を棒グラフとしてプロットし、マッププロットを使用してプロットします。
