...
-- MAGIC 
-- MAGIC ノートブックの状態はリセットされたので、レッスン用の設定を初期化する必要があります。
-- MAGIC 
-- MAGIC 注：このレッスンの後半ではデータベースは再作成**しません**。
...
-- MAGIC 
-- MAGIC しかし、今まで使用してきたデータベースがデフォルトで使用されるようにこのセッションを設定する必要があります。
...
-- MAGIC 
-- MAGIC 前述したように、テンポラリビューは、Sparkセッションに紐づかれているため、アクセスができません…
-- MAGIC * クラスタを再起動した後
-- MAGIC * クラスタのデタッチと再アタッチの跡
-- MAGIC * PythonパッケージをインストールしてPythonのインタプリタが再起動された後
-- MAGIC * または別のノートブックから
-- MAGIC 
-- MAGIC …グローバルテンポラリビューという特例を除いて。
...
-- MAGIC 
-- MAGIC クラスタを再起動すると、グローバルテンポラリビューは「失われます」。
-- MAGIC 
-- MAGIC 今はしなくても良いですが、クラスタを再起動すると、テーブルがもう存在しないため、上のSELECT文は失敗します。

-- COMMAND ----------

-- MAGIC %md <i18n value="30eadd27-c9ea-4c5c-8c9b-7cfa9476a054"/>
-- MAGIC 
-- MAGIC ## 共通テーブル式（CTE）（Common Table Expressions (CTEs)）
-- MAGIC CTEはさまざまな場所で使用されます。 以下は、CTEを照会で使用できるさまざまな方法のくつかの例です。 はじめに、CTEを使用して複数の列エイリアスを作成する例を紹介します。
